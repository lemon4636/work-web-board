...
export function APR() {
  ...
  const onAprAction = async action => {
    const res = await request(`/crm-app/updateAprConfig?action=${action}&env=${currentEnv}`, {
  ...
  const getCurrentBranch = async () => {
    const { data } = (await request(`/crm-app/getCurrentBranch`, { env: currentEnv })) || {};
  ...
  const updateCurrentBranch = async branchName => {
    const { ok, msg } =
      (await request(`/crm-app/updateCurrentBranch`, { env: currentEnv, branchName })) || {};
    message[ok ? 'success' : 'error'](msg);
    if (ok) {
  ...
  const getLocalBranchList = async () => {
    const { data } = (await request(`/crm-app/getLocalBranchList`, { env: currentEnv })) || {};
    const {
      //
      list,
    } = data || {};
  ...
  const getModifiedFiles = async () => {
    ...
    setSelectedRowKeys([]);
    setSelectedRows([]);
    const { data, ok } = (await request(`/crm-app/getModifiedFiles`, { env: currentEnv })) || {};
    const {
      //
      list,
    } = data || {};
  ...
  const updateBackupTodo = async param => {
    setTableLoading(true);
    const { ok, msg } = (await request(`/crm-app/updateBackupTodo?env=${currentEnv}`, param)) || {};
    setTableLoading(false);
    message[ok ? 'success' : 'error'](msg);
  };
  ...
  const onBackupEditModal = modalProps => {
    ...
    Modal.confirm({
      title:
        (+(backupTodoId + 1) ? '編輯No.backupTodoId_' + backupTodoId : '新增') + '：' + fileName,
      width: 800,
      icon: '',
      maskClosable: true,
      ...
      content: (
        <Form ref={backupEditFormRef} initialValues={modalProps}>
          <Form.Item name='backupFileName' label='備份文件名稱' rules={[{ required: true }]}>
            <Input placeholder='请输入' />
          </Form.Item>
          <Form.Item name='backupFileRepo' label='備份文件倉庫' rules={[{ required: true }]}>
            <Select
              placeholder='请选择'
              options={[
                {
      ...
      >
        ...
        >
          ...
          <LoadingProvider
            ...
            triggerWrapper={
              <Popconfirm title='确认操作？' onConfirm={() => updateCurrentBranch(currentBranch)} />
            }
          >
          ...
          <LoadingProvider>
            <Button
              //
              type='link'
              onClick={() => Promise.all([getCurrentBranch(), getLocalBranchList()])}
            >
              刷新
            </Button>
          </LoadingProvider>
        ...
        >
          ...
          <LoadingProvider
            triggerName='onConfirm'
            triggerWrapper={<Popconfirm title='确认操作？' onConfirm={deleteLocalBranch} />}
          >
            <Button
              type='primary'
            ...
            columns={[
              ...
              //   },
              {
                ...
                render: (val, row) => {
                  return (
                    <div>
                      <Button type='link' onClick={() => onBackupEditModal(row)}>
                        備份配置
                      </Button>
                    </div>
                  );
              ...
              },
              {
                title: '上次備份時間',
                render: (val, row) =>
                  row[`lastBackupTime_${row.currentBranch}`] || row.lastBackupTime,
              },
              {
                title: '源文件路徑',
                dataIndex: 'filePath',
